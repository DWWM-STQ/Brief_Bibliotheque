<?php
session_start();
$_SESSION['page'] = "Suppression d'un client";
...
if(isset($_POST['id']) && $_POST['id'] != null && !empty($_POST['id'])){
    ...
    if(($stmt = mysqli_prepare($conn, $sql)) && ($stmt2 = mysqli_prepare($conn, $sql2))){
        ...
        if(empty(trim($_GET['id']))) {
            header('Location: Location: ./admin_clients.php');
            exit();
        }
    }
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-dark">
                            ...
                            <p>Si vous supprimer ce client, son compte associé sera lui aussi supprimé.</p>
                            <p>
                                <input type="submit" value="Oui" class="btn btn-danger">
                                <a href="./admin_clients.php" class="btn btn-info">Non</a>
                            </p>
                        </div>
                    </form>
                </div>
